package com.ds.codegenerator.tree;

/**
 * @author jerryli
 * @description
 * @date 2019-11-07 10:38
 */
public class BSTree<T extends Comparable<T>> {
    ...
    private void insert(BSTree<T> bst, BSTNode<T> node) {
        ...
        BSTNode<T> y = null;
        // node 的插入位置
        BSTNode<T> site = bst.mRoot;

        // 查找 node 的插入位置
        while (site != null) {
            y = site;
            cmp = node.key.compareTo(site.key);
            if (cmp < 0)
                site = site.left;
            else
                site = site.right;
     ...
     *     d 删除的结点
     */
    private BSTNode<T> remove(BSTree<T> bst, BSTNode<T> d) {
        // 子节点 child
        BSTNode<T> cn = null;
        // 要删除的节点 delete
        BSTNode<T> dn = null;
        ...
        if (dn.parent == null)
            ...
            bst.mRoot = cn;
        else if (dn == dn.parent.left)
            dn.parent.left = cn;//左节点关联子节点
        else
            dn.parent.right = cn;//右节点关联子节点

        // 如果要删的节点和一开始传入的不一样，就是后继的情况
        if (dn != d)
            d.key = dn.key;//后继的值传给本来要删除的节点
        //返回被删除的节点
        return dn;
    }
    ...
    public void remove(T key) {
        ...
        if ((dn = search(mRoot, key)) != null)
            if ((node = remove(this, dn)) != null)
                node = null;
    }
